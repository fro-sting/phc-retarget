...
def fit_motion(cfg, motion_path: str, fitted_shape: torch.Tensor):
   ...
   #print(f"joint_idx={smpl_joint_idx}   robot_body_names={robot_body_names}")
    ...
    for i in range(500):
        ...
        robot_rotmat_opt = pk.quaternion_to_matrix(robot_root_quat)  # [T, 3, 3]
        ...
        #加入这几个项也会梯度爆炸
        root_quat_reg = 1e-4 * torch.mean(
            quat_error_magnitude(robot_root_quat, root_quat_init) ** 2
        )
        root_quat_vel = torch.gradient(robot_root_quat, spacing=1/cfg.target_fps, dim=0)[0]
