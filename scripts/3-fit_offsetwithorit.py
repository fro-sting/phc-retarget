...
def fit_motion(cfg, motion_path: str, fitted_shape: torch.Tensor):
    ...
    for joint_name, euler_str in cfg.orit_offset.items():
        # if joint_name == "Pelvis":
        #     continue
        joint_idx = SMPL_BONE_ORDER_NAMES.index(joint_name)
        euler_angles = eval(euler_str)
        print(f"  Joint: {joint_name} (idx={joint_idx}), euler_angles={euler_angles}")

        rot_mat = torch.as_tensor(
            sRot.from_euler("xyz", euler_angles).inv().as_matrix(),
            dtype=torch.float32,
            device=smpl_global_mats.device
        )
        # rotate the global matrix
