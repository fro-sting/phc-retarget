...
def compute_offsets(cfg):
    """compute orientation offsets between SMPL and robot model"""
    ...
    # set robot joint angles to zero
    ...
    # get robot keypoints and orientations
    root_translation = fk_output[chain.get_link_names()[0]].get_matrix()[:, :3, 3]
    ...
    print(f"Robot keypoints shape: {robot_keypoints.shape}")
    print(f"Matching {len(robot_body_names)} joints")
    
    # initialize SMPL pose parameters
    body_pose = torch.nn.Parameter(torch.zeros(1, 23, 3, dtype=torch.float32))
    global_orient = torch.nn.Parameter(torch.zeros(1, 3, dtype=torch.float32))
    transl = torch.zeros(1, 3, dtype=torch.float32)  # 不优化
    
    opt = torch.optim.Adam([body_pose, global_orient], lr=0.01)
    
    # visualization storage
    vertices_history = []
    smpl_keypoints_history = []
    
    print("Optimizing SMPL pose...")
    ITERS = 5000
    for i in range(ITERS):
        ...
        pos_loss = nn.functional.mse_loss(smpl_keypoints, robot_keypoints)
        
        # regularization to keep pose reasonable
        reg_loss = 1e-4 * (torch.sum(body_pose**2) + torch.sum(global_orient**2))
        
        loss = pos_loss + reg_loss
        
        if i % 1000 == 0 or i == ITERS - 1:
            print(f"Iter {i}, loss: {loss.item():.6f}, pos_loss: {pos_loss.item():.6f}")
            # store for visualization
            with torch.no_grad():
                v = (result.vertices[0] - result.joints[:, 0]).detach().cpu()
                vertices_history.append(v)
                smpl_keypoints_history.append(smpl_keypoints[0].detach().cpu())
        
        opt.zero_grad()
        loss.backward()
        opt.step()
    print("\nComputing orientation offsets...")
    
    # compute SMPL global rotation matrices
    with torch.no_grad():
    ...
    print("\nVisualizing results...")
    visualize_optimization(body_model, vertices_history, smpl_keypoints_history, robot_keypoints)
    
    return offsets

def visualize_optimization(body_model, vertices_history, smpl_keypoints_history, robot_keypoints):
    """visualize the optimization process using Open3D"""
    ...
    robot_keypoints_cpu = robot_keypoints.detach().cpu()

    for i, v in enumerate(vertices_history):
        offset = torch.tensor([0., i * 2, 0.])  # 每个阶段间隔开

        v_with_offset = v + offset + torch.tensor([2., 0., 0.])
        mesh = init_mesh(v_with_offset.numpy())
        vis.add_geometry(mesh)

        pcd_smpl = o3d.geometry.PointCloud()
        pcd_smpl.points = o3d.utility.Vector3dVector(
            (smpl_keypoints_history[i] + offset + torch.tensor([1., 0., 0.])).numpy()
        )
        pcd_smpl.colors = o3d.utility.Vector3dVector([[0, 0, 1] for _ in range(len(smpl_keypoints_history[i]))])
        vis.add_geometry(pcd_smpl)

        pcd_robot = o3d.geometry.PointCloud()
        pcd_robot.points = o3d.utility.Vector3dVector((robot_keypoints_cpu[0] + offset).numpy())
        pcd_robot.colors = o3d.utility.Vector3dVector([[1, 0, 0] for _ in range(len(robot_keypoints_cpu[0]))])
