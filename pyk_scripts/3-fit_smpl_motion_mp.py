...
def fit_motion(cfg, motion_path: str, fitted_shape: torch.Tensor):
    ...
    for i in range(500):
        ...
        loss = nn.functional.mse_loss(robot_keypoints_w, smpl_keypoints_w) + 0.01 * torch.mean(torch.square(robot_th))
        opt.zero_grad()
        loss.backward()
        opt.step()
        # if i % 10 == 0:
...
def main(cfg):
    ...
    joblib.dump(all_data, f"data/{cfg.humanoid_type}/v1/{cfg.output_name}.pkl")
